...
static CGFloat kBlackCoverMaxAlpha = 0.3f;
static CGFloat kRightSContentScale = 1;
...
- (id)init{
    if (self = [super init]){
        ...
        _RightSContentOffset= JRightSContentOffset;
       // kRightSContentScale=kRightSContentScale;
        _RightSContentScale=kRightSContentScale;
...
- (void)showContentNextControllerWithJModel:(NSString *)className
{
    [self closeSideBar];
    ...
    if (_mainContentView.subviews.count > 0)
    {
        ...
        [UIView animateWithDuration:0.3 animations:^{
            view.frame = CGRectMake(-SCREEN_WIDTH, 0, SCREEN_WIDTH, SCREEN_HEIGHT);
            controller.view.frame = _mainContentView.frame;
...
- (void)showContentPrevControllerWithJModel:(NSString *)className
{
    [self closeSideBar];
...
- (void)moveViewWithGesture:(UIPanGestureRecognizer *)panGes
{
    ...
    if (panGes.state == UIGestureRecognizerStateChanged)
    {
        ...
        if ((transX > 0) && _showLeftSideView)
        {
            ...
            {
                sca = 1 - (_mainContentView.frame.origin.x/_LeftSContentOffset) * (1-kRightSContentScale);
                
            }
            else
            {
                sca = kRightSContentScale;
            }
